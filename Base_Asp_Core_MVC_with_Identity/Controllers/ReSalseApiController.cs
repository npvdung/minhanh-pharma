...
namespace Base_Asp_Core_MVC_with_Identity.Controllers
{
    ...
    public class ReSalseApiController : Controller
    {
        ...
        public ReSalseApiController(Base_Asp_Core_MVC_with_IdentityContext context, UserManager<UserSystemIdentity> uid)
        {
            _uid = uid;
            _context = context;
        }
        public IActionResult Index()
        {
            try
            {
                ...
                int recordsTotal = 0;
                var customerData = from tempcustomer in _context.reSales
                ...
                {
                    customerData = customerData.Where(m => m.Sales.Contains(searchValue) || m.customName.Contains(searchValue));
                }
